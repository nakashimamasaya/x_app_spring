...
コンパイルが通っても、Claude が `git diff` を読んでから次に進む。

**同じタスクカードでも実行ごとに結果が揺れる。** `temperature=0.1` でも安定しない。
`_smoke.md`（10 行の record）で実測したところ、同一入力で次の差が出た:

- 1 回目: `import java.util.List;` あり → コンパイル成功
- 2 回目: **`import` が丸ごと抜ける** → コンパイルエラー 3 件 → `--repair` 1 回で修正成功

つまり「一度成功したから次も成功する」とは考えない。生成のたびに必ずコンパイル/テストを回す。

実際に観測された逸脱:

- **import の欠落。** 最も多い。タスクカードに書いた import すら落とすことがある。
- **タスクカードのスタイル指示を無視する。** 「コンパクトコンストラクタで」と書いても
  明示的な canonical constructor を書いてくる（コンパイルは通るので気づきにくい）。
